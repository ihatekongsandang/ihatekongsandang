...
## 🔴 추가 (17:25) — 리뷰어 06 판정·사용자 답변 반영
- 리뷰어 06 보고서 `docs/reviews/planner-05-prd-v0.5-review.md` 전문 읽고 라벨 무관 항목 반영: **J1**(원문 링크 없는 발언의 출처 최소 요건·§6② 적용 범위) · **J2 → 사용자 답변 "허용"**: 일반인 공개 계정의 논평·정리글 게시 허용, 계정명만 표기·실명 추적 금지·원문 링크 필수(CLAUDE.md 확정 표에 추가됨) — §3·§6③에 규칙화, `user-questions` 종결 · **J4**(§0.5 "사진만 받은 경우" 행) · **J5**(§6 적용 범위에 정정 절차 포함) · **J6**(원문 게시일·`attribution`·발언자 필드 필수 여부 — 단일 스키마 기준으로) · **J7**(시각 기재) · 권고 G1~G5 중 라벨 무관분. J3(closureReason)은 소멸.
- 리뷰어 06 §7 프로그래머 제약 14항 중 라벨 무관 항목을 핸드오프의 "프로그래머 03 대조표"에 포함.
